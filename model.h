...
unsigned int
TextureFromFile(const char *path, const QString &directory, bool gamma = false);
...
public:
    ...
    Model(QString const &path, bool gamma = false) : gammaCorrection(gamma) {
        loadModel(path);
    }
...
private:
    ...
    QVector<Texture>
    loadMaterialTextures(aiMaterial *mat, aiTextureType type, QString typeName) {
        QVector<Texture> textures;
...
unsigned int
TextureFromFile(const char *path, const QString &directory, bool gamma) {
